        ...
        return (
            <div className="order-panel order-waiting">
                <div className="order-panel-title">
                    <img src={require('../../asset/image/ico_waiting.png')} alt="等待支付" className="order-icon"/>等待支付
                </div>
                <div className="order-panel-desc">
                    <p>您所订购的{order.subject}还未支付，如您已经支付成功，请稍候刷新本页面重试</p>
                    <p>订单号：<span className="red">{order.id}</span></p>
                </div>
            </div>
        )
    }
    ...
    renderPayedStatus() {
        ...
        return (
            <div className="order-panel order-success">
                <div className="order-panel-title">
                    <img src={require('../../asset/image/ico_success.png')} alt="支付成功" className="order-icon"/>支付成功！
                </div>
                <div className="order-panel-desc">
                    <p>课程名称：{order.subject}</p>
                    <p>订单号：<span className="red">{order.id}</span></p>
                    <p>推荐学习方式：电脑登录 <a href="http://www.bstcine.com">www.bstcine.com</a> 进行学习</p>
                    <p className="tips">注：加微信善恩小助手（<span className="wechat-name">BSTCINE01</span>）为好友，进入本课程的学习群！</p>
                </div>
            </div>
        )
    }
    ...
    renderCanceledPayStatus() {
        ...
        return (
            <div className="order-panel order-cancel">
                <div className="order-panel-title">
                    <img src={require('../../asset/image/ico_cancel.png')} alt="已取消支付" className="order-icon"/>已取消支付
                </div>
                <div className="order-panel-desc">
                    <p>订单号：<span className="red">{order.id}</span> 已取消</p>
                </div>
            </div>
        )
    }
