...
const getHref = (task, student_id) => {
    switch (task.type) {
        case Task_Type.Video:
            return `/learn/course/${task.course_id}?task_id=${task.id}&user_id=${student_id}&lesson_id=${task.lesson_id}`;
        case Task_Type.Quiz_PDF:
            return `/learn/course/${task.course_id}?task_id=${task.id}&user_id=${student_id}&lesson_id=${task.lesson_id}`;
        case Task_Type.Quiz:
            return `/quiz/kj?task_id=${task.id}&user_id=${student_id}&cmd=check`;
        case Task_Type.Quiz_Feedback: {
            return `/quiz/kj?task_id=${task.id}&user_id=${student_id}&stats_content_quiz_id=${task.object_id}&cmd=check`;
        }
        case Task_Type.Word:
            if (task.word_start_index && task.word_end_index) {
                return `/lword?task_id=${task.id}&user_id=${student_id}&start_index=${task.word_start_index}&end_index=${task.word_end_index}`;
            } else {
                return `/lword?task_id=${task.id}&user_id=${student_id}&word_type=${task.object_id}`;
            }
    }
};
