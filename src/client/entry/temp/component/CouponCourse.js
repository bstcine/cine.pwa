...
import {
    ...
    APIURL_Temp_User_XZ_Coupon,
} from '../../../../APIConfig';
import errorMsg from '@/util/errorMsg';

class CouponCourse extends Component {
    constructor(props) {
        super(props);
        this.state = {
            phone: '',
    ...
    async submit() {
        const { phone, auth_code, coupon_no } = this.state;
        if (!commonUtil.isPhone(86, phone))
            return CMessage.info('手机格式不正确');
        if (!coupon_no) return CMessage.info('请输入优惠券码');
        this.setState({ submit_btn_disabled: true, submit_btn: '提交中' });
        const [err, res] = await fetchData(APIURL_Temp_User_XZ_Coupon, {
            phone,
            auth_code,
            coupon_no,
        });
